...
export function encryptMessage(publicKeyRecipient, message, privateKeySender) {
  ...
  const sharedSecret = ecdhSender.computeSecret(
    Buffer.from(publicKeyRecipient, "hex"),
  );
...
export function decryptMessage(
  ...
  const sharedSecret = ecdhRecipient.computeSecret(
    Buffer.from(publicKeySender, "hex"),
  );
